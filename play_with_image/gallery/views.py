...
class BaseImageListView(ListView):
    ...
    def get_queryset(self):
        ...
        return self.model.objects.filter(private=False).prefetch_related(
            'tags', 'tags__category'
        ). select_related(
            'author'
        )
...
class PhotoByListView(BaseImageListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['title'] = (
            f'Снимки сделанные {self.kwargs.get("model")}')
        return context
...
class ImageDetailView(DetailView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        ).exclude(Q(pk=self.object.pk) | Q(private=True)).prefetch_related(
            'tags', 'tags__category'
        ).select_related('author')
        # Считаем расстояние Хемминга между изображениями
        for image in related:
            hamming = hamming_distance(
                image.image_hash_part1,
                self.object.image_hash_part1)
            hamming += hamming_distance(
                image.image_hash_part2,
                self.object.image_hash_part2)
            hamming += hamming_distance(
                image.image_hash_part3,
                self.object.image_hash_part3)
            hamming += hamming_distance(
                image.image_hash_part4,
                self.object.image_hash_part4)
